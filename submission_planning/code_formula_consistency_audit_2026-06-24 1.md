...
当前代码只保留了“$\lvert H-z_k\rvert$ 越大，越模糊”的单调关系，没有显式使用 $d$ 分母、$f^2$、$N$、$d_f-f$ 或像素尺寸换算。因此：

| 项目 | 主流 CoC | 当前代码 | 一致性判断 |
|---|---|---|---|
| 深度差影响模糊 | $\lvert d-d_f\rvert$ 增大，CoC 增大 | `dist` 增大，`focus_weight` 下降 | 一致 |
...
| `surface_normals_from_um` | $\mathbf n=(-z_x,-z_y,1)/\lVert\cdot\rVert$ | 符合高度场法线定义 |
| `focus_weight` | $\exp[-0.5(\lvert H-z_k\rvert/\mathrm{DoF})^2]$ | 保留对焦层附近最清晰的单峰响应 |
...
| 差异点 | 当前代码 | 主流物理模型 | 影响 |
|---|---|---|---|
| CoC 公式 | 未显式计算 CoC | $c=b\lvert d-d_f\rvert f^2/[dN(d_f-f)]$ | 不能声称物理标定 CoC 渲染 |
